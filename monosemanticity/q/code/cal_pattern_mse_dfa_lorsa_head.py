...
with tqdm(total=total_sampled_sentences, initial=0, unit='sentences', desc=f"head {ov_head_index}") as pbar:
    while sampled_sentences < total_sampled_sentences:
        ...
        for i in range(head_act_index.shape[0]):
            ...
            k_resid_feature_acts = sae.encode(k_resid)
            ...
            k_resid_feature_tensors = sae(k_resid) - k_resid_feature_tensors
            q = q_resid @ lorsa.W_Q[qk_head_index] + lorsa.b_Q[qk_head_index]
            k = k_resid_feature_tensors @ lorsa.W_K[qk_head_index] + lorsa.b_K[qk_head_index]
            q = lorsa.apply_rotary(q.reshape(1, 1, 1, q.shape[0]).repeat([1, q_index+1, 1, 1])).squeeze()[q_index]
            k = lorsa.apply_rotary(k.reshape(k.shape[0], k.shape[1], 1, k.shape[2])).squeeze()
            feature_attention_scores = k @ q / lorsa.cfg.attn_scale
...
torch.save(max_feature_mse, f'/inspire/hdd/ws-8207e9e2-e733-4eec-a475-cfa1c36480ba/embodied-multimodality/public/zfhe/jx_projects/Lorsa/monosemanticity/q/result/pythia-160m/lorsa_head/L{layer}-{ov_head_index}.pt')