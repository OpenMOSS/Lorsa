...
with tqdm(total=total_sampled_sentences, initial=0, unit='sentences') as pbar:
    while sampled_sentences < total_sampled_sentences:
        ...
        for i in range(head_act_index.shape[0]):
            ...
            q = q_resid_feature_tensors @ model.blocks[layer].attn.W_Q[qk_head_index] + model.blocks[layer].attn.b_Q[qk_head_index]
            k = k_resid @ model.blocks[layer].attn.W_K[qk_head_index] + model.blocks[layer].attn.b_K[qk_head_index]
            q = model.blocks[layer].attn.apply_rotary(q.reshape(q.shape[0], 1, 1, q.shape[1]).repeat([1, q_index+1, 1, 1])).squeeze()[:, q_index]
            k = model.blocks[layer].attn.apply_rotary(k.reshape(1, k.shape[0], 1, k.shape[1])).squeeze()
            feature_attention_scores = q @ k.T / model.blocks[layer].attn.cfg.attn_scale
...
torch.save(max_feature_mse, f'/inspire/hdd/ws-8207e9e2-e733-4eec-a475-cfa1c36480ba/embodied-multimodality/public/zfhe/jx_projects/Lorsa/monosemanticity/q/result/pythia-160m/orig_head/L{layer}-{qk_head_index}.pt')