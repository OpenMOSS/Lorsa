...
from activations import CustomedDataset, MultiKeyDataset
...
def train_lorsa(lorsa: LowRankSparseAttention, model: HookedTransformer, cfg: LoRSATrainConfig, data: DataLoader):
    hook_in_name = f'blocks.{cfg.lorsa_config.layer}.ln1.hook_normalized'
    hook_out_name = f'blocks.{cfg.lorsa_config.layer}.hook_attn_out'
    ...
    # optimizer and scheduler
    optimizer = torch.optim.Adam(lorsa.parameters(), lr=0)
    lr_scheduler = LrWarmupScheduler(optimizer, cfg.learning_rate, cfg.lr_warm_up_tokens, cfg.total_tokens)
    k_scheduler = TopkWarmupScheduler(lorsa, cfg.start_k, cfg.end_k, cfg.k_scheduler_name, cfg.k_warm_up_tokens, cfg.total_tokens)
    lr_scheduler.update_lr(0)
    k_scheduler.update_k(0)
    
    # scale lorsa parameters
    batch = next(data_iter)
    tokens = model.to_tokens(batch, prepend_bos=cfg.lorsa_config.prepend_bos).to(cfg.device)
    tokens = tokens[:, :cfg.lorsa_config.max_length]
    ...
    with torch.no_grad():
        _, cache = model.run_with_cache(tokens, names_filter=[hook_in_name, hook_out_name])
        
        hook_in = cache[hook_in_name]
        hook_out = cache[hook_out_name]
    
        if cfg.mode == "top_k":
            if cfg.k_warm_up_tokens == 0:
                out, _ = lorsa.forward_top_k(hook_in)
            elif cfg.k_warm_up_tokens > 0:
                out = lorsa.forward(hook_in)
        elif cfg.mode == "l2":
            out, _ = lorsa.forward_l2(hook_in)
        elif cfg.mode == "default":
    ...
    while sampled_tokens < cfg.total_tokens:
        ...
        if cfg.log_to_wandb and cfg.mode == "top_k":
            tokens_count += filter_mask.sum().item()
            for i in range(cfg.lorsa_config.n_ov_heads):
                head_use_count[i] += (top_k_indices == i).sum().item()
        step += 1
        ...
        # update tqdm bar
        pbar.update(filter_mask.sum().item()) 
        pbar.set_postfix({"mse_loss": mse_loss.item(), "ev": explained_variance.mean().item()})
        pbar.refresh()
